...
def main():
    ...
    if not args.requirements_file.exists():
        raise OSError(f'Requirement file {str(args.requirements_file)} doesn\'t exist')
    if not args.requirements_file.is_file():
        raise OSError(f'Requirement file {str(args.requirements_file)} isn\'t a file')
    ...
    for file in tmp_dir.glob('*'):
        if file.is_file():
            ...
            shutil.copy(file, (args.output_dir / Path(norm_name) / package_name))
